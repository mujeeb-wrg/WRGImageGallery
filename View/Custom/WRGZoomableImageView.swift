...
open class WRGZoomableImageView: UIScrollView {
    ...
    public var landscapeAspectFill = true; // If TRUE, the WRGImageScrollView will have an 'aspect fill' behavior when in landscape orientation. Otherwise, it will be aspect fit, so that the image isn't zoomed in to fit the width of the display.
...
//        zoomView?.frame = frame
    ...
    fileprivate func zoomRectForScale(_ scale: CGFloat, center: CGPoint) -> CGRect {
        ...
        // at a zoom scale of 1.0, it would be the size of the WRGImageScrollView's bounds.
